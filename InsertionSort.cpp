...
//Find the correct position of the element by binarysearch
void InsertionSort::BinaryInsertionSortAlgorithm(int* array, int size)
{
    for(int i = 1; i < size; i++)
    {
	int loc = BinarySearch(array, 0, i - 1, *(array + i));
	pairwiseSwap(array, i, loc); // used for pairwise elements
    }
    /* Use this for shifting the elements instead of pairwise swaps
    for(int i = 1; i < size; i++)
    {
